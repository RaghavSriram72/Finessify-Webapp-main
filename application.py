...
from helpers import login_required, getArticle
...
def login():
    ...
    if request.method == "POST":
        ...
        if len(rows) != 1 or not check_password_hash(rows[0]["password"], request.form.get("pass")):
...
#    articles = getArticle()
    return render_template("guides.html", username = username) #articles = articles)
...
def todoApp():
    ...
    todo_list = db.execute("""SELECT * FROM todo WHERE username = :username""", username=username)
...
def add():
    ...
    db.execute("""
                INSERT INTO todo (username, title, start_time, complete)
                VALUES (:username, :title, :start_time, :complete)""",
                username=username, title=title, start_time=str(request.form.get("time")), complete=False)
